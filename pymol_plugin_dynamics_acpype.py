...
class Gromacs_input:
    ...
    def x2top(self, file_path, project_name):
        ...
        if os.path.isfile(file_path+".top") == True and stop == 0:
            status = ["ok", "Calculating structure using x2top."]
        else:
    ...
    ##This is alternative function for small molecules to create initial topology and triectory using pdb file
    def acpype(self, file_path, project_name):
        status = ["ok", "Calculating topology using Force fields\nby ACPYPE - wait a moment"]
        status_update(status)
        
        try:
            os.remove(project_name+".gro")
            os.remove(project_name+".top")
            os.remove(project_name+"_GMX.itp")
        except:
            pass
        
        command = "acpype -i "+project_name+".pdb -o gmx -l"
        executeAndMonitorSubprocess(command, None, 'log1.txt', 'log.txt')
        
        acpype_fpath = file_path+".acpype/"+project_name 
        if os.path.isfile(acpype_fpath+"_GMX.top") == True and stop == 0:
            status = ["ok", "Calculated topology using acpype."]
            shutil.copy(acpype_fpath+"_GMX.top",file_path+".top")
            shutil.copy(acpype_fpath+"_GMX.gro",file_path+".gro")
            shutil.copy(acpype_fpath+"_GMX.itp",file_path+"_GMX.itp")
        else:
            status = ["fail", "Unable to create topology file."]
        return status
...
def steps_configure(master, restraints_button):
    if project_name != "nothing":
        ...
        r3 = Radiobutton(frame1, text="Use acpype tool", value=2, variable=v1, command=lambda: progress.x2top_update(v1.get()))
...
def dynamics():
    ...
    elif status[0] == "ok" and stop == 0 and progress.to_do[1] == 1 and progress.x2top == 2:
        status = gromacs2.acpype(file_path, project_name)
        if status[0] == "ok":
            progress.status[1] = 1
            progress.to_do[1] = 0
            save_options()
...
def load_options():
    ...
    print("Loading project " + project_name + " projekt_dir: " + str(project_dir))
